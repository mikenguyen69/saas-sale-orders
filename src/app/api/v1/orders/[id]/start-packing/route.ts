...
/**
 * @swagger
 * /api/v1/orders/{id}/start-packing:
 *   post:
 *     summary: Start packing order
 *     description: Start packing an approved order. Only warehouse staff can start packing orders.
 *     tags: [Orders]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *               notes:
 *                 type: string
 *                 description: Additional notes for starting packing
 *                 maxLength: 1000
 *     responses:
 *       200:
 *         description: Order packing started successfully
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                 message:
 *                   type: string
 *                   example: Order packing started successfully
 *       400:
 *         description: Order cannot be moved to packing
 *         content:
 *           application/json:
 *             schema:
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Only warehouse staff can start packing orders
    ...
    if (order.status !== 'approved') {
      throw new ApiError(400, 'Only approved orders can be moved to packing')
    }
    ...
    return createSuccessResponse(updatedOrder, 'Order packing started successfully')
  } catch (error) {
