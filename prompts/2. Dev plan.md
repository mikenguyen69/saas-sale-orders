...
## Phase 4: Frontend Development (6-8 days)

**Goal**: Build interactive UI with role-based views. Modularize components for reusability with robust state management and error handling.

- **Sub-Modules**:
  - **Auth and Layout (1 day)**:
    - Login page with Supabase Auth and loading states
    - Shared layout with role-based navigation
    - Error boundaries for auth failures
    - Protected route wrapper components

  - **State Management Setup (0.5 days)**:
    - Implement global state management (Context API or Zustand)
    - Set up error state handling patterns
    - Create loading state management utilities

  - **Product Listing (1 day)**:
    - Page to view/search products (use React Query for fetching)
    - Implement skeleton loaders and virtualization for large catalogs
    - Add pagination and infinite scroll support
    - Search debouncing and filtering

  - **Order Form (2.5-3 days)**:
    - Core page with MUI DataGrid for line items
    - Implement editing, highlighting out-of-stock, and buttons (Save Draft, Submit) conditionally by role
    - Use React Hook Form + Yup for validation with real-time feedback
    - Handle file uploads (Supabase Storage) for attachments:
      - Progress indicators and file validation
      - Drag-and-drop interface
      - Error handling for upload failures
    - Form state persistence (auto-save drafts)
    - Real-time stock validation

  - **Order List and Details (1.5-2 days)**:
    - Dashboard for listing orders (filtered by role) with advanced filtering
    - Details view with workflow buttons (Approve, Reject, Fulfill)
...
- **Quality Assurance**:
  ...
  - Error boundary testing and fallback UI components
  - Loading state testing and skeleton loader verification
  - Ensure a11y: Add ARIA labels, test keyboard navigation, screen reader compatibility
  - Mobile testing: Use browser dev tools for responsiveness, touch interactions
  - Optimistic updates with React Query for smooth UX (e.g., instant status changes)
  - Performance testing: Large dataset rendering, code splitting verification
  - Form validation edge cases and error recovery scenarios
